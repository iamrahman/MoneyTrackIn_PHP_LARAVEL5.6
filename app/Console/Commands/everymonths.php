...
use App\Periodic;
use App\Transaction;
class everymonths extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'every:month';
     ...
     * @var string
     */
    protected $description = 'Periodic Transaction: Monthly';
    ...
    public function handle()
    {
        ...
        $amount = array();
        $i=0;
        $data = Periodic::where('duration',3)->get();
        foreach ($data as $da){
            $tags[$i] = $da->tags;
        ...
        for($i=0;$i<count($data);$i++){
            ...
            $tran->description = "Periodic Transaction after every month";
            $tran->tags = $tags[$i];
            $tran->amount = $amount[$i];
            $tran->date = date("Y-m-d")." ".time("h:m:s");
